...
from src.automata.finite_state_machine import DFA
from src.core.rl_core import RLcore
...
def train(
    ...
    learning_task = RLcore(MDP, LDBA, discount_factor, learning_rate, decaying_learning_rate, epsilon)

    if algorithm == 'ql':
        learning_task.train_ql(episode_num, iteration_num_max)
    elif algorithm == 'pi':
        learning_task.train_pi(episode_num, iteration_num_max)
    elif algorithm == 'ddpg':
        learning_task.train_ddpg(episode_num, iteration_num_max, replay_buffer_size)
        import tensorflow as tf
        tf.get_logger().setLevel('ERROR')
    else:
        raise NotImplementedError('New learning algorithms will be added soon. The selected algorithm is not '
                                  'implemented yet.')
    ...
    if test:
        ...
        for tt in range(number_of_tests):
            ...
            iteration_num = 0
            while (not LDBA.accepting(LDBA.automaton_state)) and iteration_num < iteration_num_max \
                ...
                if not algorithm == "ddpg":
                    ...
                    next_automaton_state = learning_task.DFA.step(learning_task.MDP.state_label(next_MDP_state))
                    if ndarray:
            ...
            if LDBA.accepting(next_automaton_state):
    ...
    if isinstance(MDP, SlipperyGrid) and test:
        ...
        labels_values = np.zeros([learning_task.MDP.shape[0], learning_task.MDP.shape[1], 100])
        for av in range(100):
            MDP.reset()
            for i in range(learning_task.MDP.shape[0]):
                for j in range(learning_task.MDP.shape[1]):
                    labels_values[i][j][av] = labels_dic[learning_task.MDP.state_label([i, j])]
