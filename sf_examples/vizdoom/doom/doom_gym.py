...
import threading
from .wrappers.safeguard_callbacks import SAFEGUARD_CALLBACKS_LIST, SAFETY_CHECKS
import pathlib

def write_to_file_with_lock(file, text):
    base = './violation_outputs/'
    pathlib.Path(base).mkdir(parents=True, exist_ok=True)
    with open(base + file, 'a') as logfile:
        logfile.write(text)
...
class VizdoomEnv(gym.Env):
    def __init__(
        ...
        self.intrinsic_fear = fear
        self._FEAR_RADIUS = fear_radius
        self._FEAR_STEPS = fear_radius
        self._FEAR_PENALTY = fear_penalty
        self._FEARING = False
        self.prev_vars = {}
        ...
        self.trial = trial
        self.penalty_violation = penalty_violation
        
        self.safeguard_callbacks = SAFEGUARD_CALLBACKS_LIST
        ...
        # violations
        self.violation_1 = False
        self.violation_2 = False
        self.fsm_state = 0
        self.step_count_ = 0
        self.decorrelating = False
        self.prev_vars = {}
    ...
    def set_decorrelating(self, flag):
        self.decorrelating = flag
    ...
    def _game_init(self, with_locking=True, max_parallel=5):    
        lock_file = lock = None
        if with_locking:
        ...
        init_attempt = 0
        while True:
            def reset_lock(lock_file, max_parallel=5):
                import os
                os.remove(lock_file)
                lock_file = doom_lock_file(max_parallel)
                lock = FileLock(lock_file)
            ...
            try:
                ...
                if with_locking:
                    ...
                    if init_attempt > 1:
                        ...
                        lock_file, lock = reset_lock(lock_file)
    ...
    def reset(self, **kwargs) -> Tuple[np.ndarray, Dict]:
        ...
        self.violation_2 = False
        self.fsm_state = 0
        self._FEAR_STEPS = self._FEAR_RADIUS
    ...
    def step(self, actions) -> Tuple[np.ndarray, float, bool, bool, Dict]:
        ...
        def lava_check_other(pos_x, pos_y):
            if pos_x >= 160:
                return True
            else:
                return False

        def lava_check(pos_x, pos_y):
            if pos_y > -224:
                return True
            else:
                return False
            
        def boss_check(pos_x, pos_y):
            if pos_y > 96:
                return True
            else:
                return False
        ...
        prev_vars = self.prev_vars.get("HEALTH", 100)
        ...
        violation_penalty = self.penalty_violation
        
        # SAFEGUARDS
        if self.safeguard != -1:
            safeguard = self.safeguard_callbacks[self.safeguard]

            flag, self.fsm_state = safeguard(info, lava_loc, boss_loc, self.fsm_state)
        ...
        # INTRINSIC FEAR
        if prev_vars < info["HEALTH"]:
            self._FEARING = True
            
        if self.intrinsic_fear > 0 and self._FEARING:
            if self._FEAR_STEPS > 0:
                reward -= self._FEAR_PENALTY
                self._FEAR_STEPS -= 1
        else:
            self._FEARING = False
            self._FEAR_STEPS = self.intrinsic_fear
                
        # EPISODIC VIOLATIONS
        if not self.decorrelating:
            if not self.violation_1:
                if SAFETY_CHECKS["in_lava_without_armor"](info, lava_loc):
                    self.violation_1 = True
                    if self.prefix == 'transfer':
                        write_to_file_with_lock(f"{self.prefix}_episodic_violations_{self.trial}.txt", f"T{self.safeguard} {self.step_count_} {self._num_episodes} 1\n")
                    else:
                        write_to_file_with_lock(f"{self.prefix}_episodic_violations_{self.trial}.txt", f"{self.safeguard} {self.step_count_} {self._num_episodes} 1\n")
            
            if not self.violation_2:
                if self.main_level and SAFETY_CHECKS["in_boss_without_gun"](info, boss_loc):
                    self.violation_2 = True
                    if self.prefix == 'transfer':
                        write_to_file_with_lock(f"{self.prefix}_episodic_violations_{self.trial}.txt", f"T{self.safeguard} {self.step_count_} {self._num_episodes} 2\n")
                    else:
                        write_to_file_with_lock(f"{self.prefix}_episodic_violations_{self.trial}.txt", f"{self.safeguard} {self.step_count_} {self._num_episodes} 2\n")
          
        
        # PER FRAME VIOLATIONS
        if not self.decorrelating:
            # violation condition #1: dont go into lava
            if SAFETY_CHECKS["in_lava_without_armor"](info, lava_loc):
                if self.prefix == 'transfer':
                    write_to_file_with_lock(f"{self.prefix}_violations_{self.trial}.txt", f"T{self.safeguard} {self.step_count_} {self._num_episodes} 1\n")
                else:
                    write_to_file_with_lock(f"{self.prefix}_violations_{self.trial}.txt", f"{self.safeguard} {self.step_count_} {self._num_episodes} 1\n")

            # violation condition #2: dont fight boss without a gun
            if self.main_level and SAFETY_CHECKS["in_boss_without_gun"](info, boss_loc):
                if self.prefix == 'transfer':
                    write_to_file_with_lock(f"{self.prefix}_violations_{self.trial}.txt", f"T{self.safeguard} {self.step_count_} {self._num_episodes} 2\n")
                else:
                    write_to_file_with_lock(f"{self.prefix}_violations_{self.trial}.txt", f"{self.safeguard} {self.step_count_} {self._num_episodes} 2\n")
